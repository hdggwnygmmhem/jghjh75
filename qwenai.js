import axios from 'axios';
import crypto from 'crypto';
import { cmd } from '../command.js'; // اپنے کمانڈ ہینڈلر کا صحیح پاتھ (path) لکھیں

// ==========================================
//          QWEN AI SCRAPER FUNCTION
// ==========================================
export async function qwenchat(message) {
    if (!message) {
        throw new Error('Pesan tidak boleh kosong.');
    }

    const timestamp = Date.now();
    const requestId = crypto.randomUUID();

    try {
        const payload = {
            chatId: "",
            models: ["qwen3.7-plus"],
            project_id: "",
            timestamp: timestamp,
            chat_type: "t2t",
            chat_mode: "guest"
        };

        const headers = {
            'Accept': 'application/json, text/plain, */*',
            'Accept-Language': 'en-US,en;q=0.9',
            'Content-Type': 'application/json',
            'Version': '0.2.87',
            'source': 'h5',
            'Timezone': new Date().toString(),
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
            'Origin': 'https://chat.qwen.ai',
            'Referer': 'https://chat.qwen.ai/',
            'bx-umidtoken': 'T2gAbheReRRQHQREJVinIV71av3i7ot6pOw4-aGlkCMoJP_3Q-npd7CvjvEWL6LLwP8=',
            'bx-v': '2.5.37',
            'bx-ua': '234!',
            'X-Request-Id': requestId
        };

        const sessionResponse = await axios.post('https://chat.qwen.ai/api/v2/chats/new', payload, { headers });

        if (!sessionResponse.data?.success || !sessionResponse.data?.data?.id) {
            throw new Error('Gagal membuat sesi chat baru: ' + JSON.stringify(sessionResponse.data));
        }

        const chatId = sessionResponse.data.data.id;
        const completionPayload = {
            stream: true,
            version: "2.1",
            incremental_output: true,
            chatId: chatId,
            parentId: "",
            chat_id: chatId,
            chat_mode: "guest",
            model: "qwen3.7-plus",
            parent_id: null,
            messages: [
                {
                    id: null,
                    fid: crypto.randomUUID(),
                    parentId: null,
                    childrenIds: [],
                    role: "user",
                    content: message,
                    user_action: "chat",
                    files: [],
                    timestamp: Math.floor(timestamp / 1000),
                    models: ["qwen3.7-plus"],
                    model: "",
                    chat_type: "t2t",
                    feature_config: {
                        thinking_enabled: true,
                        output_schema: "phase",
                        research_mode: "normal",
                        auto_thinking: true,
                        thinking_mode: "Auto",
                        thinking_format: "summary",
                        auto_search: true
                    },
                    extra: {
                        meta: { subChatType: "t2t" }
                    },
                    sub_chat_type: "t2t",
                    parent_id: null
                }
            ],
            timestamp: Math.floor(Date.now() / 1000)
        };

        const streamHeaders = {
            ...headers,
            'Accept': 'text/event-stream',
            'X-Request-Id': crypto.randomUUID()
        };

        const response = await axios.post(
            `https://chat.qwen.ai/api/v2/chat/completions?chat_id=${chatId}`,
            completionPayload,
            { headers: streamHeaders, responseType: 'stream' }
        );

        return new Promise((resolve, reject) => {
            let fullAnswer = '';
            let buffer = '';

            response.data.on('data', (chunk) => {
                buffer += chunk.toString();
                const lines = buffer.split('\n');
                buffer = lines.pop() || '';

                for (const line of lines) {
                    const trimmedLine = line.trim();
                    if (trimmedLine.startsWith('data: ')) {
                        const jsonStr = trimmedLine.replace(/^data:\s*/, '');
                        if (jsonStr === '[DONE]') continue;

                        try {
                            const parsed = JSON.parse(jsonStr);
                            const choices = parsed.choices;
                            if (choices && choices.length > 0) {
                                const delta = choices[0].delta;
                                if (delta && delta.phase === 'answer' && delta.content) {
                                    fullAnswer += delta.content;
                                }
                            }
                        } catch {
                            // Ignored partial parse error
                        }
                    }
                }
            });

            response.data.on('end', () => {
                resolve(fullAnswer.trim());
            });

            response.data.on('error', (err) => {
                reject(new Error('Stream error: ' + err.message));
            });
        });

    } catch (error) {
        if (error.response) {
            throw new Error(`HTTP Error [${error.response.status}]: ${JSON.stringify(error.response.data)}`);
        }
        throw error;
    }
}

// ==========================================
//          WHATSAPP BOT COMMAND
// ==========================================
cmd({
    pattern: "qwen",
    alias: ["qwenai"],
    desc: "Qwen AI Model Chat",
    category: "ai",
    filename: import.meta.url
},
async (conn, mek, m, { q, reply, react }) => {
    try {
        if (!q) {
            await react("❌");
            return reply("⚠️ *براہ کرم کوئی سوال یا میسج لکھیں!*\n\n*Example:* `.qwen Siapakah kamu?`");
        }

        await react("🧠");

        const responseText = await qwenchat(q);

        if (!responseText) {
            await react("❌");
            return reply("❌ *AI کی طرف سے کوئی جواب نہیں ملا۔*");
        }

        await reply(responseText);
        await react("✅");

    } catch (e) {
        console.error("Qwen Error:", e);
        await react("❌");
        await reply(`❌ *Error:* ${e.message}`);
    }
});
