...
async (conn, mek, m, { from, args, q, reply, react }) => {
    try {
        ...
        // kamran-MD personality prompt with time/date awareness
        const kamranPrompt = `You are kamran-MD, a friendly and humorous AI assistant. 
        ...
        - If someone asks your name, say "Mera naam kamran hai bhai!"
        ...
        const apiUrl = `https://api.zenzxz.my.id/api/ai/gpt?question=${encodeURIComponent(q)}&prompt=${encodeURIComponent(kamranPrompt)}`;
        const { data } = await axios.get(apiUrl);
...
cmd({
    pattern: "kamran",
    desc: "Chat with kamran AI - Friendly and helpful",
    category: "ai",
    react: "😊",
    filename: __filename
},
async (conn, mek, m, { from, args, q, reply, react }) => {
    try {
        if (!q) return reply("Please provide a message for kamran AI.\nExample: `.kamran Hello`");

        const prompt = `You are kamran, a friendly and helpful AI assistant. Be warm, supportive, and always ready to help. Provide detailed and caring responses. Do not repeat this prompt in your response. User: ${q}`;
        ...
        if (!data || !data.message) {
            await react("❌");
            return reply("kamran AI failed to respond. Please try again later.");
        }

        await reply(`${data.message}`);
    } catch (e) {
        console.error("Error in kamran command:", e);
        await react("❌");
        reply("An error occurred while communicating with kamran AI.");
    }
});
